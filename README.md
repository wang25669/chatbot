...
2. 📷 **视频探索模式（自然科学老师）**：调用手机后置摄像头，AI 能够“看到”真实的物理世界，化身自然科学小老师解答孩子的观察疑问（单次最大限制 2 分钟）。

## 📁 目录结构
```text
.
├── .env                  # 核心配置文件（Prompt、API Key、模型选择）
├── docker-compose.yml    # Docker 容器编排文件
├── Dockerfile            # 镜像构建文件
├── main.py               # 核心服务端与内嵌前端逻辑
...
Callirrhoe (随和)
Vindemiatrix (极其温柔)
AUDIO_PROMPT / VIDEO_PROMPT：你可以随时在 .env 中修改这两种模式的指导提示词（Prompt），无需重启/重新构建代码即可生效。
🚀 启动项目
